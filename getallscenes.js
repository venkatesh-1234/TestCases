 ...
 beforeEach(function (done) {
   ...
   request.post('/Login', {
    ...
    it('Geting all scenes with valid details', function (done) {
        ...
        request.get('/scenes/getAll',{'userID':userid},tempvar, function (err, res, body) {
           var output = JSON.parse(body)
           assert.equal(res.statusCode,200)
           assert.equal(output.success,true)
           assert.equal(output.Result['2551234567'].Scenes,'')
            console.log("output--->",output.Result['2551234567'].Scenes)
